...
const MatrixBG = (props) => {
  ...
  const chars =
    "アァカサタナハマヤャラワガザダバパイィキシチニヒミリヰギジヂビピウゥクスツヌフムユュルヱグズヅブペエェケセテネヘメレヱゲゼデベペオォコソトノホモヨョロヲゴゾドボポヴッン0123456789";
  const letters = chars.split("");

  // Font-size remains constant, so similarly move outside draw
  const fontSize = 40;
  // Setting up the columns
  const columns = canvasWidth ? canvasWidth / fontSize : 40;
  ...
  const draw = () => {
    if (ctx) {
      ...
      for (let i = 0; i < drops.length; i++) {
        ...
        ctx.fillStyle = "#00F0FF"; // Neon Cyan/Blue
